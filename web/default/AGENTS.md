...
- **组件分层**：`src/components/ui/` 保存 shadcn/Base UI 原始组件，`src/components/design-system/` 只封装稳定的产品级策略。受 `.oxlintrc.json` 管控的组件必须从 `design-system` 导入，不得绕过；不要为没有跨页面策略的组件增加代理层。
- **上游更新**：更新 shadcn 组件时先使用 `bunx --bun shadcn@latest add <component> --dry-run` 和 `--diff` 审查，再合并到 `components/ui`；未经明确批准不得使用 `--overwrite`。
- **语义样式**：状态颜色使用 `success`、`warning`、`destructive`、`info`、`neutral` 等语义令牌；文本徽章统一使用 `StatusBadge`。后台业务组件不得用硬编码调色板颜色表达状态。
- **排版**：正文使用 `--font-sans`，并保留 CJK 回退字体；`font-mono` 仅用于密钥、ID、代码和原始 JSON，普通数字对齐使用 `tabular-nums`。
- **控件尺寸**：受管控件通过语义化 `size` 选择尺寸，业务调用点不得用 `h-*`、`size-*` 或 `p-0` 绕过适配层。默认控件在手机端为 28px、`sm` 及以上为 32px；密集场景使用固定 `sm`，微型场景使用固定 `xs`。
- **圆角与层级**：圆角来自 `--radius` 及其 Tailwind 派生档；普通卡片使用边框或投影之一，投影主要保留给浮层。
- **动效**：后台页面切换只使用透明度 fade，并尊重 `prefers-reduced-motion`；禁止恢复 stagger、位移、缩放或模糊入场。装饰性动效只放在营销首页。
...
- **2026-07-13**：补充设计系统边界，固化 shadcn/Base UI 源码层、产品适配层、语义样式、响应式尺寸与后台动效约束。

...
**上游同步标记：** 当本仓库吸收上游 `QuantumNous/new-api` 的某条提交时：

- 本仓库主线中的吸收提交应使用 `sync: ...` 风格描述同步内容
- 上游提交、本地主线中的 `sync:` 提交和 `sync/*` tag 必须严格一一对应
- 禁止把多条上游提交聚合、压缩或 squash 成一条本地 `sync:` 提交，也禁止让多个 `sync/*` tag 指向同一条本地提交
- 多条上游提交存在依赖时，应按依赖顺序分别同步、分别验证、分别提交和分别打 tag；如果无法安全拆分，则整组停止自动同步并等待决策
- 必须为每条已吸收的上游提交创建 Git tag，新 tag 格式统一为：`sync/<完整上游提交SHA>`；历史遗留的唯一短 SHA tag 只做兼容识别，不再新增
- 该 tag 指向本仓库主线里对应的 `sync:` 提交，而不是直接指向上游提交对象
- 创建 tag 前必须确认目标本地提交尚未被其他 `sync/*` tag 占用；创建后必须检查每条本地 `sync:` 提交最多只有一个 `sync/*` tag
- 只重复父提交内容、没有独立补丁的上游 merge 提交，不得在已有本地提交上追加第二个 tag；应在同步账本中记录其 `covered_by` 关系
- 不要使用 `sync/功能名`、`sync/日期` 等不便于反查上游来源的命名
- 若上游提交未被吸收，则不要预先创建对应 `sync/*` tag
